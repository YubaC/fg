...
function next() {
    if (enable_text_touch) {
        ...
        if (!textok) {
            ...
            speakerAt < paraList.length - 1 &&
            paraList[speakerAt].type != "ask"
        ) {
            //否则已经打完字了 => 继续下一句话
            speakerAt += 1;
            if (paraList[speakerAt].type == "say") {
        ...
        if (paraList[speakerAt].type == "exit") {
            //如果下一句话是type="exit"（要求退出对话）
            hideDialog();
            hideChoice();
            if (speakerAt < paraList.length - 1) {
                //如果这条命令后还有命令 => 一定是要执行函数
                speakerAt += 1;
                // next(); //执行这个函数
            }
        }
    ...
    timer = setInterval(() => {
        if (str_.length < str.length) {
            ...
            if (document.querySelector("#choice li").clientWidth < 100) {
                for (
                    i = 0;
                    i < document.querySelectorAll("#choice li").length;
                    i++
                ) {
                    document.querySelectorAll("#choice li")[i].style.width =
                        "100px";
                }
            }
            // }, 500);
...
function receiveSave(fileInputed) {
    ...
    reader.onload = function (evt) {
        ...
        // 检查存档是否完好
        if (
            fileString.search("version") != -1 &&
            JSON.parse(fileString).version == flow.version
        ) {
            console.log("in3-");
            ...
            for (i = 0; i < loadedSave.point_list.length; i++) {
                point_list.push(
                    document.querySelector(loadedSave.point_list[i])
                );
            }
...
function showDay() {
    ...
    document.querySelector(
        "#day p"
    ).innerHTML = `今日空气污染：${airPollution}&emsp;学期日${todayInTerm}/${term}`;
...
function getCSSPath(node) {
    ...
    while (node.parentElement) {
        ...
        let siblingsArr = Array.prototype.slice.call(
